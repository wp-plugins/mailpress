...
////  Styles  ////
	...
	{
		wp_register_style ( self::screen, get_option('siteurl') . '/' . MP_PATH . 'mp-admin/css/mails.css',       array('thickbox') );
...
// url's
		...
		unset($args['action']);
		$tracking_url 	= clean_url(self::url( MailPress_tracking, array_merge($args, $url_parms)));
...
// actions
		...
		if (!apply_filters('MailPress_is_tracking', false))	unset($actions['tracking']);
...
//	to
		...
		switch (true)
		{
			...
			case (self::is_email($mail->toemail)) :
				...
				if ( ('detail' == $url_parms['mode']) && (get_option('show_avatars') ) ) $email_display .= get_avatar( $mail->toemail, 32 );
