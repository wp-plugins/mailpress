...
//// List ////
	...
	public static function get_list($start, $num, $url_parms) 
	{
		...
		$libs = array( 'all' => __('All'), 'draft' =>	__('Draft', MP_TXTDOM), 'unsent' => __('Unsent', MP_TXTDOM), 'sending' => __('Pending', MP_TXTDOM), 'sent' => __('Sent', MP_TXTDOM), 'search' => __('Search Results') );
...
// url's
		...
		$args['action'] 	= 'send';
		$send_url 		= clean_url(self::url( MailPress_write, array_merge($args, $url_parms), "send-mail_$id" ));

		$args['action'] 	= 'delete';
		$delete_url 	= clean_url(self::url( MailPress_write, array_merge($args, $url_parms), "delete-mail_$id" ));

		unset($args['action']);
		if (apply_filters('MailPress_is_tracking', false)) $tracking_url 	= clean_url(self::url( MailPress_tracking_m, array_merge($args, $url_parms)));
