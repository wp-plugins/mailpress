...
////  Redirect  ////
	...
	public static function redirect() 
	{
		...
		if (empty($_GET['delete_users'])) return;		// MANAGING CHECKBOX REQUESTS
...
// url's
		...
		unset($args['action']);
		if (class_exists('MailPress_tracking')) $tracking_url = clean_url(self::url( MailPress_tracking_u, array_merge($args, $url_parms)));
...
// actions
		...
		if (class_exists('MailPress_tracking')) $actions['tracking'] = "<a href='$tracking_url' title='" . __('See tracking results', MP_TXTDOM ) . "'>" . __('Tracking', MP_TXTDOM) . '</a>';
		...
		if (!current_user_can('MailPress_tracking_users')) 	unset($actions['tracking']);
...
//// Body ////
	...
	public static function users_restrict($url_parms)
	{
		...
		if ($letters) $list[0] = __('Starting with...', MP_TXTDOM);
		foreach ($letters as $letter) $list[$letter->letter] = $letter->letter;

		$selected = (isset($url_parms['startwith'])) ? $url_parms['startwith'] : 0;
