...
<?php self::select_option($xssl,$smtp_config['ssl']);?>
					</select>
					&nbsp;
...
<?php self::select_option($xport,$smtp_config['port']);?>
					</select>
					&nbsp;
			...
			<tr>
				<th>
					<?php _e('Pop before Smtp','MailPress'); ?>   
				</th>
				<td> 
					<select name='smtp_config[smtp-auth]' id='smtp-auth'>
<?php self::select_option($xauth,$smtp_config['smtp-auth']);?>
					</select>
				</td>
				<td id='POP3'<?php if ('@PopB4Smtp' != $smtp_config['smtp-auth']) echo " style='display:none;'"; if (isset($pophostclass)) echo " class='form-invalid'"; ?>> 
					<?php _e("POP3 hostname",'MailPress'); ?>
					&nbsp;&nbsp;
					<input type='text' size='25' name='smtp_config[pophost]' value='<?php echo $smtp_config['pophost']?>' />
					<?php _e("port",'MailPress'); ?>
					&nbsp;&nbsp;
					<input type='text' size='4' name='smtp_config[popport]' value='<?php echo $smtp_config['popport']?>' />
				</td>
			</tr>
		</table>
