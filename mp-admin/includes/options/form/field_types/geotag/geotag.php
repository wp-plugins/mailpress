...
class MP_Forms_field_type_geotag extends MP_Forms_field_type_abstract
{
	...
	function attributes_filter($no_reset)
	{
		...
		if (isset($options['geocode'])) $options['geocode_value'] =  MailPress::input_text(stripslashes($_POST[$this->prefix][$this->field->form_id][$this->field->id]['geocode']));
	...
	// javascript
		...
		if (!defined('MP_FORM_GEOTAG'))
		{
			...
			$js .= ";\n/* ]]> */\n</script>";
			//$js .= "\n<script type='text/javascript' src='" . get_option('siteurl') . '/' . MP_PATH . "mp-includes/js/mp_gmap.js'></script>\n";
...
$MP_Forms_field_type_geotag = new MP_Forms_field_type_geotag();
?>