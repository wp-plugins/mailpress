...
//		$_SESSION['cryptogra']['use']++;
	...
	function create($style, $settings)
	{
		...
		{
			$dh = opendir($bgimg);
			while (($filename = readdir($dh)) !== false) if (eregi(".[gif|jpg|png]$", $filename)) $files[] = $filename;
			closedir($dh);
...
$MP_Forms_field_type_captcha_gd1_cryptogra = new MP_Forms_field_type_captcha_gd1_cryptogra();
?>