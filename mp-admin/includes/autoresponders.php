...
$total = ( isset($url_parms['s']) && !empty($url_parms['s']) ) ? count(MP_Autoresponders::get_all(array('hide_empty' => 0, 'search' => $url_parms['s']))) : wp_count_terms(self::taxonomy);
if( !isset($autorespondersperpage) || $autorespondersperpage <= 0 ) $autorespondersperpage = 20;

$page_links = paginate_links	(array(	'base' => add_query_arg( 'apage', '%#%' ),
							'format' => '',
							'total' => ceil( $total / $autorespondersperpage),
...
<?php self::get_list( (isset($url_parms['apage'])) ? $url_parms['apage'] : 1, $autorespondersperpage ); ?>
						</tbody>
					</table>
					<div class='tablenav'>
<?php 	if ( $page_links ) echo "						<div class='tablenav-pages'>$page_links</div>\n"; ?>
