...
switch($action) 
{
	case 'view' :
		...
		if ('draft' == $mail->status)
		{
			...
			$form .="<input type='hidden' name='subject' value='" . $mail->subject . "' />\n";
			$form .="<input type='hidden' name='html'    value='" . $mail->html    . "' />\n";
			$form .="<input type='hidden' name='plaintext'    value='" . $mail->plaintext    . "' />\n";
	...
	break;
	case 'draft' :
		if ((empty($_POST['toemail'])) && (!is_numeric($_POST['to_list'])))
		{
		...
		elseif (!empty($_POST['toemail']))
		{
			if(!MailPress::is_email($_POST['toemail']))
			{
				$message  .=	__('Please, enter a valid email',  'MailPress')		. '<br />'; 
				$style = " style ='background-color:#f00;color:#fff;'";
				$err   = false;
		...
		switch(true)
		{
			case (isset($_POST['view'])) :
				...
				$html = $x->process_img($html,$x->mail->themedir,'html');
