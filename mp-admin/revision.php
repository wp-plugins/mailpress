...
////  Redirect  ////
	...
	{
		$action	= (isset($_GET['action'])) 	? absint($_GET['action']) : false;
		$revision_id= (isset($_GET['revision'])) 	? absint($_GET['revision']) : false;
		$id		= absint($_GET['id']);
		$diff		= (isset($_GET['diff'])) 	? absint($_GET['diff']) : false;
...
////  Styles  ////
	...
	{
		wp_register_style ( self::screen, get_option('siteurl') . '/' . MP_PATH . 'mp-admin/css/write.css', array('thickbox') );
