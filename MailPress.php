...
require_once(MP_TMP . 'mp-admin/class/MP_abstract.class.php');
...
// for wp admin
		...
		//	add_action('load-post.php', 		array('MailPress', 'load_post'));
		//	add_action('load-post-new.php', 	array('MailPress', 'load_post'));
...
//		if (version_compare($wp_version, $min_ver_wp , '<'))	$m[] = sprintf(__('Your %1$s version is \'%2$s\', at least version \'%3$s\' required.', 'MailPress'), __('WordPress'), $wp_version , $min_ver_wp );
		...
		include (MP_TMP . 'mp-admin/includes/install/install.php');
	}
...
////  Settings  ////
	...
	public static function plugin_action_links($links, $file)
	{
		...
		$settings_link = '<a href="' . MailPress_settings . '#fragment-0">' . __('Settings') . '</a>';
...
////  Roles and capabilities  ////
	...
	{
		include (MP_TMP . 'mp-admin/includes/capabilities/capabilities.php');
...
// for global css
		...
		$css_url		= get_option('siteurl') . '/' . MP_PATH . 'mp-admin/css/colors_' . get_user_option('admin_color') . '.css';
		$css_url_default 	= get_option('siteurl') . '/' . MP_PATH . 'mp-admin/css/colors_fresh.css';
