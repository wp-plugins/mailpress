...
wp_reset_vars(array('revision', 'id', 'left', 'right', 'action'));
...
{
	case 'diff' :
		$left_revision  	= MP_Mail::get_mail( $left );
		$right_revision 	= MP_Mail::get_mail( $right);
		$mail    		= MP_Mail::get_mail($id);

		$write_url  = MailPress_write . '&id=' . $id;
		$write_url  = clean_url($write_url);
		$mail_title = '<a href="' . $write_url . '">' . $mail->subject . '</a>';
		$h2 = sprintf( __( 'Compare Revisions of &#8220;%1$s&#8221;','MailPress' ), $mail_title );

		$left  = $left_revision->id;
		$left_link = ('' == $left_revision->status) ? MailPress_revision . '&id=' . $mail->id . '&revision=' . $left : MailPress_write . '&id=' . $left;

		$right = $right_revision->id;
		$right_link = ('' == $revision->status) ? MailPress_revision . '&id=' . $mail->id . '&revision=' . $right : MailPress_write . '&id=' . $right;

		$date = mp_mail_revision_title( $rev_id, $link );
	break;
	default :
		$revision 	= MP_Mail::get_mail( $revision_id );
		$mail 	= MP_Mail::get_mail( $id );

		$write_url  = clean_url(MailPress_write . '&id=' . $id);
		$mail_title = '<a href="' . $write_url . '">' . $mail->subject . '</a>';
