...
class MailPress_sync_wordpress_user extends MP_abstract
{
	...
	public static function profile_update($wid)
	{
		...
		if ($id)
		{
			...
			if ($_POST['email'] != $mp_user->email)
			{
				...
				if (self::is_email($_POST['email'])) self::sync($wp_user);
			}
		}
		else
		{
...
// so insert !
	...
	{
		if ( !self::is_email($wp_user->user_email) )					return false; // not an email
...
// for meta box in user page
	public static function styles($styles, $screen) 
	{
		...
		wp_register_style ( 'MailPress_sync_wordpress_user', 	get_option('siteurl') . '/' . MP_PATH . 'mp-admin/css/user_sync_wordpress_user.css' );
