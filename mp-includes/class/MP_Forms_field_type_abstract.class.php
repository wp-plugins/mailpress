...
abstract class MP_Forms_field_type_abstract
{
	...
	{
		if ($no_reset) $this->field->settings['attributes']['value'] = MailPress::input_text(stripslashes($_POST[$this->prefix][$this->field->form_id][$this->field->id]));
	...
	// other attributes
		foreach ($this->field->settings['attributes'] as $attribute => $value)
		{
			...
			if ('misc'        == $attribute) { $misc = trim(stripslashes($value)); if ('' != $misc) $misc = " $misc"; continue; }
	...
	function get_attribute($attr, $value) { $value = stripslashes($value); $quote = (false !== strpos($value, '"')) ? "'" : '"'; return " $attr=$quote$value$quote"; 	}
...
<?php
	...
	function settings_hidden($setting, $attribute) { ?><input type='hidden' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' value="<?php if ($this->type_ok && isset($this->field->settings[$setting][$attribute])) echo MP_AdminPage::input_text($this->field->settings[$setting][$attribute]); ?>" /><?php }
	function settings_hidden_value($setting, $attribute, $value) { ?><input type='hidden' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' value="<?php echo $value; ?>" /><?php }
	function settings_text($setting, $attribute, $disabled = false, $default = '', $size = 32) { ?><input type='text' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_' . $setting . '_' . $attribute; ?>' value="<?php echo ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? MP_AdminPage::input_text($this->field->settings[$setting][$attribute]) : $default ); ?>" size='<?php echo $size; ?>' /><?php }
	function settings_checkbox($setting, $attribute, $value, $disabled = false, $class = false, $forced = false) { ?><input type='checkbox' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_' . $setting . '_' . $attribute . '_' . $value; ?>' value="<?php echo $value; ?>" style='width:auto;'<?php if ($disabled) echo " disabled='disabled'"; ?><?php if ($class) echo " class='$class'"; ?><?php if ($forced !== false) checked(true); else checked( $value, ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? $this->field->settings[$setting][$attribute] : null ) ); ?> /><?php if ($forced !== false) return true; return ($this->type_ok && isset($this->field->settings[$setting][$attribute]) && ($value == $this->field->settings[$setting][$attribute]) );}
	function settings_radio($setting, $attribute, $value, $value_text, $default, $disabled = false)	{ ?><input type='radio'	name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type .'_settings_' . $setting . '_' . $attribute . '_' . $value; ?>' 	value="<?php echo $value; ?>" style='width:auto;'<?php checked($value, ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? $this->field->settings[$setting][$attribute] : $default) ); ?><?php if ($disabled) echo " disabled='disabled'"; ?> /><label class='inline' for='<?php echo $this->prefix . $this->field_type .'_settings_' . $setting . '_' . $attribute . '_' . $value; ?>' ><span class='description'><small<?php if ($disabled) echo " style='color:#888'";?>><?php echo $value_text; ?></small></span></label>&nbsp;<?php }
	...
	function settings_attributes_textarea($text, $option) { ?><input type='hidden' name='<?php echo $this->prefix . $this->field_type . '[textareas][]'; ?>' value='<?php echo $option; ?>' /><?php $this->settings_description($text); ?><br /><textarea name='<?php echo $this->prefix . $this->field_type . "[settings][attributes][$option]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_attributes_' . $option; ?>' cols='40' rows='4'><?php if ($this->type_ok && isset($this->field->settings['attributes']['tag_content'])) echo MP_AdminPage::input_text(trim(stripslashes(base64_decode($this->field->settings['attributes']['tag_content'])))); ?></textarea><?php }
}
?>