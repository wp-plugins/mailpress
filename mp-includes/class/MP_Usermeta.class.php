...
class MP_Usermeta
{
	...
	public static function get_by_id( $uid ) 
	{
		...
		if ($meta && is_serialized_string( $meta->meta_value ) )	$meta->meta_value = maybe_unserialize( $meta->meta_value );
		return $meta;
	}
...
//		$meta_key = preg_replace('|[^a-z0-9_]|i', '', $meta_key);
	...
	public static function add_meta($mp_user_id)
	{
		$mp_user_id = (int) $mp_user_id;
		...
		if ( ('0' === $metavalue || !empty ( $metavalue ) ) && ((('#NONE#' != $metakeyselect) && !empty ( $metakeyselect) ) || !empty ( $metakeyinput) ) ) 
		{
			...
			return self::add( $mp_user_id, $metakey, $metavalue );
		}
		return false;
	}

	public static function get_replacements($id) 
	{
		$replacements = array ();
		if (!$id) return $replacements;

		$metas = self::get( $id );
		if (!$metas) return $replacements;
