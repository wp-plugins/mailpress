...
add_action('mp_action_theme-preview',	array('MP_Themes','mp_action_theme_preview'));
add_action('mp_action_previewtheme',	array('MP_Themes','mp_action_previewtheme'));
...
//		return true;
	...
	public static function mp_action_theme_preview() 
	{
		$preview_url= get_option('siteurl') . '/' . MP_PATH . "mp-includes/action.php";
		$html_url 	= add_query_arg( array('action' => 'previewtheme', 'template' => $_GET['template'], 'stylesheet' => $_GET['stylesheet'], 'type' => 'html'), $preview_url);
		$plaintext_url = add_query_arg( array('action' => 'previewtheme', 'template' => $_GET['template'], 'stylesheet' => $_GET['stylesheet'], 'type' => 'plaintext'), $preview_url);

		$html 	= "<iframe id='ihtml'      style='width:100%;border:0;height:550px' src='" . clean_url($html_url) . "'></iframe>";
		$plaintext 	= "<iframe id='iplaintext' style='width:100%;border:0;height:550px' src='" . clean_url($plaintext_url) . "'></iframe>";

		include MP_TMP . '/mp-includes/theme-preview.html.php';
	}

	public static function mp_action_previewtheme() 
	{
		global $mp_general;

		$url 		= get_option('siteurl');

		$args->Theme 		= $_GET['template'];;
		...
		$args->unsubscribe = __('"Subscription management link"','MailPress');
		$args->viewhtml    = __('"Trouble reading link"','MailPress');
		...
		if ('html' == $_GET['type'])
		{
			$x->args->html = stripslashes(apply_filters('the_content', $x->html));
			$x->html 	   = $x->build_mail_content('html');
			echo $x->process_img($x->html,$x->mail->themedir,'draft');
		}
		else
		{
			$x->args->plaintext 	= stripslashes($x->plaintext);
