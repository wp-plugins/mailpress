...
class MP_Forms_templates
{
	...
	{
   		// Load all templates.
		$root = ABSPATH . MP_PATH . 'mp-content/forms';
		$dir  = @opendir($root);
		if ($dir) 
			while (($file = readdir($dir)) !== false) if (($file{0} != '.') && (substr($file, -4) == '.xml')) 
			{
//if (defined('MP_SCRIPT_DEBUG')) { global $mp_debug_log; if (isset($mp_debug_log)) $mp_debug_log->log(" คค loading $file ..." ); }
				$f = file_get_contents("$root/$file");
//if (defined('MP_SCRIPT_DEBUG')) { global $mp_debug_log; if (isset($mp_debug_log)) $mp_debug_log->log(" คคค loaded $file" ); }
