...
class MP_Log
{
	...
	function MP_Log($name,$path,$plug = '',$force=false)
	{
		$this->data = "\n";
		...
		$this->level 	= (isset($mp_general['level']))    ? (int) $mp_general['level'] 	: MP_Log::noMP_Log ;
...
// purge log
		$now = date('Ymd');
		$this->lastpurge 	= (isset($mp_general['lastpurge'])) ? $mp_general['lastpurge'] 		: $now;
		$this->lognbr 	= (isset($mp_general['lognbr']))    ? (int) $mp_general['lognbr'] 	: 1;
		
		if ($now != $this->lastpurge) 
		{
			$this->dopurge ($now);
			$mp_general['lastpurge'] = $now;
			if (!add_option ('MailPress_general', $mp_general, 'MailPress - general' ))
