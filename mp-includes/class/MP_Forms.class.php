...
class MP_Forms
{
		...
		switch (true)
		{
			...
			default :
				...
				elseif ( ! $_form = wp_cache_get($form, 'mp_form') ) 
				{
					...
					$_form->settings = unserialize(stripslashes($_form->settings));
	...
	public static function get_all() 
	{
		...
		$query = "SELECT * FROM $wpdb->mp_forms a ORDER BY label";
	...
	public static function insert($_post_form) 
	{
		...
		$_post_form = wp_parse_args($_post_form, $_post_defaults);
	...
	// is visitor
		...
		{
			$where['id'] 	= (int) $id;
			$wpdb->update( $wpdb->mp_forms, $data, $where );
		}
		else
		{
	...
	public static function duplicate($id)
	{
		...
		$data['label'] 		= mysql_real_escape_string(stripslashes($form->label));		$format[] = '%s';
		$data['description'] 	= mysql_real_escape_string(stripslashes($form->description));	$format[] = '%s';
		$data['template']		= mysql_real_escape_string(stripslashes($form->template));		$format[] = '%s';
		$data['settings'] 	= mysql_real_escape_string(serialize($form->settings));		$format[] = '%s';	
		global $wpdb;
		$wpdb->insert( $wpdb->mp_forms, $data, $format );
	...
	{
		global $wpdb;
		$query = "DELETE FROM $wpdb->mp_forms WHERE id = $id;";
		$wpdb->query($query);
...
//// the form /building/resetting/controlling 
	...
	public static function form( $id )
	{
		...
		if (!$form) return '<!-- MailPress_form  ERROR : unknown form #' . $form->id . ' ! -->';
...
//// Processing form
	public static function process( $form, $fields, $form_templates)
	{
		...
		if ('0' != $form->settings['visitor']['subscription'])
		{
			MailPress::require_class('Users');
			if ( (isset($visitor_toemail)) && (!MP_Users::get_id_by_email($visitor_toemail)) )
			{
		...
		// no valid mail
		if (!MailPress::is_email($visitor_toemail)) return false;
...
//// Building form
	...
	// misc
		$misc = (isset($attributes['misc'])) ? ' ' . stripslashes($attributes['misc']) : '';
	...
	public static function get_attribute($attr, $value) { $value = stripslashes($value); $quote = (false !== strpos($value, '"')) ? "'" : '"'; return " $attr=$quote$value$quote"; }
	...
	public static function handle_map($source) 
	{
		...
		if (copy($source, MP_TMP . $dest)) { fclose(fopen(MP_TMP . $dest, 'r+')); return $dest;}
		return false;
	}
