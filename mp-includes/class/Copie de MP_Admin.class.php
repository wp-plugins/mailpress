...
//// scripts ////
	...
	public static function register_scripts() {
		...
		wp_register_script( 'mp-ajax-response', 		'/wp-includes/js/wp-ajax-response.js', array('jquery'), false, 1);
		...
		wp_register_script( 'mp_swf_upload', '/' . MP_PATH . 'mp-includes/js/fileupload/swf.js', array('swfupload'));
	// these error messages came from the sample swfupload js, they might need changing.
		...
		wp_register_script( MailPress_page_write, 	'/' . MP_PATH . 'mp-admin/js/mail_new.js', array('jquery-ui-tabs', 'jquery-color', 'thickbox', 'mp-autosave', 'mp-lists', 'postbox', $mp_upload_script ),false,true);
...
// for appropriate javascript
	...
	public static function enqueue_scripts($is_footer) {
		...
		$footer_enabled = (version_compare($GLOBALS['wp_version'], '2.8') === 1) ? true : false;
...
/*
		$footer_enabled = (version_compare($GLOBALS['wp_version'], '2.8', '<')) ? false : true;
		$action = ($footer_enabled) ? 'admin_footer' : 'admin_head'; 


if (!$footer_enabled) echo '(false && ';
else echo '(true && ';
if (!$is_footer) echo 'false) ';
else echo 'true) ';
echo " $action";
		if ($footer_enabled && !$is_footer) return; 
