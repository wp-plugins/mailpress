...
//// Tracking events to autorespond to  ////
	...
	function callback($mp_user_id)
	{
		...
		foreach( $autoresponders as $autoresponder )
		{
			...
			wp_schedule_single_event($schedule, 'mp_process_autoresponder_' . $this->id, 	array('args' => array('umeta_id' => $umeta_id, 'mail_order'=> 0 )));

			MailPress::require_class('Log');
			$this->trace = new MP_Log('MP_Autoresponder_' . $term_id, ABSPATH . MP_PATH, 'autoresponder', false, 'autoresponder');
...
//		$this->trace->log("***                  *** autoresponder_id : $term_id, mp_user_id : $mp_user_id");
		...
		wp_schedule_single_event($schedule, 'mp_process_autoresponder_' . $this->id, array('args' => array('umeta_id' => $umeta_id, 'mail_order'=> $mail_order)));
