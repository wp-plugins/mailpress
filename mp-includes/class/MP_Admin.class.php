...
//// scripts ////
	...
	public static function register_scripts() {
		...
		wp_register_script( 'mp-ajax-response', 		'/wp-includes/js/wp-ajax-response.js', array('jquery'), false, 0);
		wp_localize_script( 'mp-ajax-response', 		'wpAjax', array( 'noPerm' => __('Email was not sent AND/OR Update database failed','MailPress'), 'broken' => __('An unidentified error has occurred.'), 'l10n_print_after' => 'try{convertEntities(wpAjax);}catch(e){};' ) );

		wp_register_script( 'mp-autosave', 			'/' . MP_PATH . 'mp-includes/js/autosave.js', array('schedule', 'mp-ajax-response'), false, 0);
		...
		wp_register_script( 'mp-lists', 			'/' . MP_PATH . 'mp-includes/js/mp-lists.js', array('mp-ajax-response'), false, 0);
		wp_localize_script( 'mp-lists', 			'wpListL10n', array( 'url' => get_option( 'siteurl' ) . '/' . MP_PATH . 'mp-includes/action.php' ) );

		wp_register_script( 'mp_swf_upload', '/' . MP_PATH . 'mp-includes/js/fileupload/swf.js', array('swfupload'), false, 0);
	// these error messages came from the sample swfupload js, they might need changing.
		...
		wp_register_script( 'mp_html_sifiles', '/' . MP_PATH . 'mp-includes/js/fileupload/si.files.js', array(), false, 0);
		wp_register_script( 'mp_html_upload', '/' . MP_PATH . 'mp-includes/js/fileupload/htm.js', array('mp_html_sifiles'), array(), false, 0);
		...
		wp_register_script( MailPress_page_mails, 	'/' . MP_PATH . 'mp-admin/js/mails.js', array('thickbox', 'mp-lists'), false, 0);
		wp_localize_script( MailPress_page_mails, 	'adminmailsL10n', array('pending' => __('%i% pending'),
														'screen' => $mp_screen ) );

		wp_register_script( MailPress_page_mail, 		'/' . MP_PATH . 'mp-admin/js/mail.js', array('jquery-ui-tabs'), false, 0);

		$w = array('quicktags', 'mp-autosave', 'mp-lists', 'postbox');
		if ( user_can_richedit() )	$w[] = 'editor';
		$w[] = 'thickbox';
		$w[] = (MP_Mail::flash()) ? 'mp_swf_upload' : 'mp_html_upload';
		wp_register_script( MailPress_page_write, 	'/' . MP_PATH . 'mp-admin/js/mail_new.js', $w, false, 0);
		wp_localize_script( MailPress_page_write, 	'mailnewL10n', array( 	'errmess' => __('Enter a valid email !','MailPress'),
														'screen' => $mp_screen ) );

		wp_register_script( MailPress_page_design, 	'/' . MP_PATH . 'mp-admin/js/themes.js', array( 'thickbox', 'jquery' ), false, 0);

		wp_register_script( MailPress_page_settings, 	'/' . MP_PATH . 'mp-admin/js/settings.js', array('jquery-ui-tabs'), false, 0);

		wp_register_script( MailPress_page_users, 	'/' . MP_PATH . 'mp-admin/js/users.js', array('mp-lists'), false, 0);
		wp_localize_script( MailPress_page_users, 	'adminusersL10n', array('pending' => __('%i% pending'),
														'screen' => $mp_screen ) );

		wp_register_script( MailPress_page_user, 		'/' . MP_PATH . 'mp-admin/js/user.js', array('jquery-ui-tabs', 'postbox'), false, 0);
