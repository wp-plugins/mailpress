...
//// scripts ////
	...
	public static function register_scripts() {
		...
		$mp_upload_script = (MP_Mail::flash()) ? 'mp_swf_upload' : 'mp_html_upload';

		wp_register_script( 'mp-ajax-response', 		'/wp-includes/js/wp-ajax-response.js', array('jquery'));
		...
		wp_register_script( 'mp-autosave', 			'/' . MP_PATH . 'mp-includes/js/autosave.js', array('schedule', 'mp-ajax-response'));
		...
		wp_register_script( 'mp-lists', 			'/' . MP_PATH . 'mp-includes/js/mp-lists.js', array('mp-ajax-response'));
		wp_localize_script( 'mp-lists', 			'wpListL10n', array( 'url' => get_option( 'siteurl' ) . '/' . MP_PATH . 'mp-includes/action.php' ) );


		wp_register_script( MailPress_page_mails, 	'/' . MP_PATH . 'mp-admin/js/mails.js', array('thickbox', 'mp-lists'));
		wp_localize_script( MailPress_page_mails, 	'adminmailsL10n', array('pending' => __('%i% pending'),
														'screen' => $mp_screen ) );

		wp_register_script( MailPress_page_mail, 		'/' . MP_PATH . 'mp-admin/js/mail.js', array('jquery-ui-tabs'));

		wp_register_script( MailPress_page_write, 	'/' . MP_PATH . 'mp-admin/js/mail_new.js', array('jquery-ui-tabs', 'jquery-color', 'thickbox', 'mp-autosave', 'mp-lists', 'postbox', $mp_upload_script ));
		wp_localize_script( MailPress_page_write, 	'mailnewL10n', array( 	'errmess' => __('Enter a valid email !','MailPress'),
														'screen' => $mp_screen ) );

		wp_register_script( 'mp_swfupload', '/wp-includes/js/swfupload/swfupload.js');
		wp_register_script( 'mp_swfupload-swfobject', '/wp-includes/js/swfupload/plugins/swfupload.swfobject.js');
		wp_register_script( 'mp_swfupload-queue', '/wp-includes/js/swfupload/plugins/swfupload.queue.js');
		wp_register_script( 'mp_swf_upload', '/' . MP_PATH . 'mp-includes/js/fileupload/swf.js', array('mp_swfupload','mp_swfupload-swfobject','mp_swfupload-queue'));
...
/*
		wp_register_script( 'mp_swfupload-degrade', '/wp-includes/js/swfupload/plugins/swfupload.graceful_degradation.js', array('smp_wfupload'), '2.2.0-20081031');
		wp_localize_script( 'mp_swfupload-degrade', 'uploadDegradeOptions', array(
			'is_lighttpd_before_150' => is_lighttpd_before_150(),
		) );
*/
		wp_register_script( 'mp_html_sifiles', '/' . MP_PATH . 'mp-includes/js/fileupload/si.files.js');
		wp_register_script( 'mp_html_upload', '/' . MP_PATH . 'mp-includes/js/fileupload/htm.js', array('mp_html_sifiles'));
		...
		wp_register_script( MailPress_page_design, 	'/' . MP_PATH . 'mp-admin/js/themes.js', array( 'thickbox', 'jquery' ));

		wp_register_script( MailPress_page_settings, 	'/' . MP_PATH . 'mp-admin/js/settings.js', array('jquery-ui-tabs'));

		wp_register_script( MailPress_page_users, 	'/' . MP_PATH . 'mp-admin/js/users.js', array('mp-lists'));
		wp_localize_script( MailPress_page_users, 	'adminusersL10n', array('pending' => __('%i% pending'),
														'screen' => $mp_screen ) );

		wp_register_script( MailPress_page_user, 		'/' . MP_PATH . 'mp-admin/js/user.js', array('jquery-ui-tabs', 'postbox'));
		wp_localize_script( MailPress_page_user, 		'adminuserL10n', array('screen' => $mp_screen ) );
...
// for appropriate javascript
	...
	public static function enqueue_scripts() {
		...
		if (MP_Mail::flash()) add_action('admin_head', array('MP_Mail','swfupload'));
	}
