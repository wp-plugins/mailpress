<?php
add_action('activate_' . MP_FOLDER . '/MailPress.php',	array('MP_Admin','install'));
...
//// roles and capabilities ////
	...
	public static function mailpress_write() 		{ include (MP_TMP . '/mp-admin/mail-new.php'); }
			...
			case (MailPress_page_revision == $page) :						// MANAGING REVISION
				...
				switch ( $action )
				{
					...
					case 'diff' :
						...
						$rev_ids = MP_Mail::get_meta($id,'_MailPress_mail_revisions');
...
//// styles ////
	...
	public static function register_styles() {
		...
		wp_register_style ( MailPress_page_write, 	get_option('siteurl') . '/' . MP_PATH . 'mp-admin/css/mail-new.css', 	array('thickbox') );
...
//// scripts ////
	...
	public static function register_scripts() {
		...
		$mp_screen = MP_Admin::screen_meta_screen($hook_suffix);
		...
		wp_register_script( MailPress_page_write, 	'/' . MP_PATH . 'mp-admin/js/mail-new.js', array('jquery-ui-tabs', 'jquery-color', 'thickbox', 'mp-autosave', 'mp-lists', 'postbox'));
...
// for appropriate javascript
	...
	public static function enqueue_scripts() {
		...
		if (( MailPress_page_write == $page) || ( MailPress_page_edit == $page)) add_action( 'admin_head', 'wp_tiny_mce' );
	}
		...
		switch ($page)
		{
			case MailPress_page_write :
				...
				if (isset($_GET['id']))
				{
					...
					$rev_ids 	= MP_Mail::get_meta($draft->id,'_MailPress_mail_revisions');
				}
				if ($rev_ids)
...
// for parms & urls
	...
	public static function get_url_parms($parms = array('mode','status','s','apage','author'))
	{
		foreach ($parms as $v)
		{
...
// for select
	...
	public static function select_option($list, $selected)
	{
...
<?php
	...
	public static function select_number($start,$max,$selected,$tick=1)
	{
		while ($start <= $max)
		{
