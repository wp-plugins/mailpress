...
// for newsletters
	...
	public static function register_newsletters()
	{
		self::register (	'new_post', 
					'', 
					false, 
					'single', 
		...
		if (isset($mp_subscriptions['default_newsletters']))
		{
			global $mp_registered_newsletters;
...
// for sending mails
	...
	public static function mailinglists( $draft_dest = array() ) 
	{
		...
		foreach ($mp_registered_newsletters as $k => $v) if (isset($subscriptions['newsletters'][$k])) $draft_dest["MailPress_newsletter~$k"] = $v['desc'];
		return $draft_dest;
	}
...
// for shortcode
	...
	public static function process() 
	{
		...
		foreach ($newsletters as $k => $v)
		{
			...
			if ($newsletter['threshold'])
			{
				...
				{
					self::prepare_to_send($newsletter);
				}
			}
		}
	}
	...
	{
		global $wpdb;
		$query = "	SELECT DISTINCT a.id, a.email, a.name, a.status, a.confkey 
	...
	public static function prepare_to_send($newsletter, $qp = true) 
	{
		...
		self::send($newsletter, $mail, $qp);
	}

	public static function mp_action_mp_meta_box_post() 
	{
		global $user_ID;
	...
	public static function send($newsletter, $mail, $qp = true) 
	{
		if (!isset($newsletter['query_posts'])) return;

		$rc = true;

		$mail->Theme 	= $newsletter['theme'];
		if (!$newsletter['theme'] || empty($newsletter['theme'])) unset($mail->Theme);
	...
	public static function have_post($post_id)
	{
		...
		self::prepare_to_send($newsletter, false);
	}
	...
	public static function get_active() 
	{
		...
		foreach ($x as $k => $v) if (!isset($mp_subscriptions['newsletters'][$k]))		unset($x[$k]);
	...
	public static function get_default_newsletters() 
	{
		...
		foreach ($x as $k => $v) if (!isset($mp_subscriptions['default_newsletters'][$k])) 	unset($x[$k]);
...
//		mp_theme 		: MailPress theme for this newsletter (if you create it do not forget plaintext) **since 4.0**
...
//
//
function mp_register_newsletter($id, $mp_subject, $mp_template, $desc, $display = true, $threshold = false, $in = false, $args = false, $mp_theme = false ) 
{
	MP_Newsletter::register($id, $mp_subject, $mp_theme, $mp_template, $desc, $display = true, $threshold = false, $in = false, $args = false);
}
?>