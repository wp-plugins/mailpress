...
//		mp_theme 		: MailPress theme for this newsletter (if you create it do not forget plaintext)
...
//
//
function mp_register_newsletter($id, $mp_subject, $mp_theme, $mp_template, $desc, $display = true, $threshold = false, $in = false, $args = false) 
{
...
class MP_Newsletter extends MP_abstract
{
// for newsletters
...
// for shortcode
	...
	public static function mp_action_mp_meta_box_post() 
	{
		...
		$x = new WP_Ajax_Response( array	(
								'what' => 'mp_post_test',
								'id' => $mail->id,
								'data' => !$rc ? __('Sending mail failed !', 'MailPress') : sprintf('<span id="mail-%1$s">%2$s</span>',$mail->id , sprintf(__('%1$sView%2$s sent mail', 'MailPress'), sprintf('<a href="%1$s" class="thickbox">', clean_url(self::url( MP_Action_url, array('action' => 'iview', 'id' => $mail->id, 'KeepThis' => 'true', 'TB_iframe' => 'true')))), '</a>'))
								)
						 );
		$x->send();
	}
