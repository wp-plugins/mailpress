...
//		else do_action('mp_action_newslettersend');
	...
	function get_mp_user_newsletters($mp_user_id = false) {

		global $mp_registered_newsletters;

		$x = MP_Newsletter::get_active();
		$a = ($mp_user_id) ? MP_User::get_usermeta($mp_user_id,'_MailPress_newsletter') : '';
	...
	function checklist_mp_user_newsletters($mp_user_id = false, $args = '') {
	...
	function update_mp_user_newsletters($mp_user_id, $name = 'keep_newsletters') {

		global $mp_registered_newsletters;

		$x = MP_Newsletter::get_active();
		MP_User::delete_usermeta($mp_user_id,'_MailPress_newsletter');

		foreach ($x as $k => $v) 
		{
			if ($mp_registered_newsletters[$k]['in']) 
			{
				if (isset($_POST[$name][$k])) MP_User::add_usermeta($mp_user_id,'_MailPress_newsletter',$k);
			}
			else
			{
				if (!isset($_POST[$name][$k])) MP_User::add_usermeta($mp_user_id,'_MailPress_newsletter',$k);
