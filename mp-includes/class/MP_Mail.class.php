...
// mail
	...
	function send_draft($id=NULL,$ajax=false,$_toemail=false,$_toname=false) {
...
<?php
			if ($attach) :
?>
			<img class='attach' alt='' src='<?php echo get_option('siteurl') . '/' . MP_PATH; ?>/mp-includes/images/clip.gif'/>
<?php
			endif;
?>
			<strong>
...
/// THICKBOX ///
	...
	public static function mp_action_iview()
	{
		...
		$id 		= $_GET['id'];
		$main_id	= (isset($_GET['main_id'])) ? "&main_id=" . $_GET['main_id'] : "&main_id=" . $id;
		$mail 	= self::get($id);
		$mp_general = get_option('MailPress_general');
		...
		$html_url 	= clean_url(get_option('siteurl') . '/' . MP_PATH . "mp-includes/action.php?action=viewadmin&id=$id$main_id&type=html$theme");
		$html 	= "<iframe id='ihtml' style='width:100%;border:0;height:550px' src='" . $html_url . "'></iframe>";
		$plaintext_url = clean_url(get_option('siteurl') . '/' . MP_PATH . "mp-includes/action.php?action=viewadmin&id=$id$main_id&type=plaintext$theme");
