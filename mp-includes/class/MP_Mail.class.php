...
// toemail & toname
		...
		elseif ( (isset($this->args->toemail)) && (!empty($this->args->toemail)) )
		{
			...
			$this->row->toname  = (empty($this->args->toname))  ? MP_Mail::display_name($this->args->toemail) : MP_Mail::display_name($this->args->toname);
			$this->batch = false;
		}
		else
		{
		...
		try 
		{
			switch (true)
			{
				case (true  === $this->batch) :
					...
					if (!$this->swift->batchSend($this->message, $this->to , $this->from))
					{
						$this->swift->disconnect(); 
						$this->mysql_connect('A.2');
						return false;
					}

					$this->mysql_connect('A.1');
				break;
				case (false === $this->batch) :
					$this->mysql_disconnect('B');

					if (!$this->swift->send($this->message, $this->to , $this->from)) 
					{
						$this->swift->disconnect();
						$this->mysql_connect('B.2');
						return false;
					}

					$this->swift->disconnect();
					$this->mysql_connect('B.1');
				break;
				default :
...
//			$this->mysql_connect('Swift Error');
			return false;
		}
		return true;
	}
			...
			switch ($_GET['tg'])
			{
				case ('l') :
					switch ($meta->meta_value)
					{
						...
						break;
						default :
							$url = str_replace('&#038;', '&', str_replace('&amp;', '&', $meta->meta_value));
						break;
					}
					wp_redirect($url);
					die();
