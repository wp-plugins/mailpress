...
// toemail & toname
		...
		elseif ( (isset($this->args->toemail)) && (!empty($this->args->toemail)) )
		{
			...
			$this->row->toname 	= (MailPress::is_email($this->row->toname)) ? str_replace('@','(at)',$this->row->toname) : $this->row->toname;
			$this->batch = false;
		}
		else
		{
...
// to 
		...
		else
		{
			...
			foreach ($this->row->toemail as $dest)	$this->to->addTo($dest['{{toemail}}'],(MailPress::is_email($dest['{{toemail}}'])) ? str_replace('@','(at)',$dest['{{toemail}}']) : $dest['{{toemail}}']);
		}
// subject 
...
////	MP_Mail SMTP functions	////
	...
	public static function SMTP_connect($x)
	{
...
////	MP_Mail mail functions	////
	...
	function display_toemail($toemail,$toname,$tolist='')
	{
		...
		{
				if (empty($toname))						return "$toemail"; 
				if (MailPress::is_email($toname))				return str_replace('@','(at)',$toname) . " &lt;$toemail&gt;"; 
...
////	MP_Mail meta functions	////
	...
	public static function delete_mail_meta( $mp_mail_id, $meta_key = '' , $meta_value = '' ) 
	{
		global $wpdb;
	...
	public static function get_mail_meta( $mp_mail_id, $meta_key = '', $meta_value = '') 
	{
		global $wpdb;
	...
	public static function add_mail_meta( $mp_mail_id, $meta_key, $meta_value ) 
	{
		global $wpdb;
		if ( !is_numeric( $mp_mail_id ) ) return false;
		$meta_key = preg_replace('|[^a-z0-9_]|i', '', $meta_key);
...
// FIXME: mp_mailmeta data is assumed to be already escaped
	...
	public static function update_mail_meta($mail_id, $meta_key, $meta_value, $prev_value = '') 
	{
		global $wpdb;
...
////	MP_Mail mail-new mp_action functions	////
	...
	function mp_action_iview()
	{
		...
		$mail 	= MP_Mail::get_mail($id);
		$mp_general = get_option('MailPress_general');
		$from 	= ('send' == $mail->status) ? MP_Mail::display_toemail($mail->fromemail,$mail->fromname) :  MP_Mail::display_toemail($mp_general['fromemail'],$mp_general['fromname']);
