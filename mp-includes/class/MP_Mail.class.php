...
$this->trace->log('**** SWIFT ERROR ****' . "There was a problem with this image: $file \n" . $e->getMessage());	
		...
		foreach ($matches as $match)
		{
			...
			if ('html' != $dest) $match[3]  = apply_filters('MailPress_process_img',$match[3]);
...
////	MP_Mail meta functions	////
	...
	function delete_mailmeta( $mp_mail_id, $meta_key, $meta_value = '' ) 
	{
		global $wpdb;
		...
		else					$wpdb->query( $wpdb->prepare("DELETE FROM $wpdb->mp_mailmeta WHERE mail_id = %d AND meta_key = %s", $mp_mail_id, $meta_key) );

		return true;
	}

	function get_mailmeta( $mp_mail_id, $meta_key = '', $meta_value = '') 
	{
		global $wpdb;
	...
	function add_mailmeta( $mp_mail_id, $meta_key, $meta_value ) 
	{
		global $wpdb;
		if ( !is_numeric( $mp_mail_id ) ) return false;
		$meta_key = preg_replace('|[^a-z0-9_]|i', '', $meta_key);
...
// FIXME: mp_mailmeta data is assumed to be already escaped
		...
		if (empty($meta_value)) return delete_mailmeta($mp_mail_id, $meta_key);
...
// Set mail's subject and body
	...
	function update_draft($id)
	{
		...
		$query = "UPDATE $wpdb->mp_mails SET status = 'draft', theme = '', toemail = '" . trim($_POST['toemail']) . "', toname = '" . trim($_POST['toname']) . "', subject = '" . trim($_POST['subject']) . "', html = '" . trim($_POST['content']) . "', plaintext = '" . trim($_POST['plaintext']) . "', created = '$now', created_user_id = " . MailPress::get_wp_user_id() . " WHERE id = $id ;";
