					...
					if ($mail)
					{
						if (MailPress::is_email($mail->toemail))
						{
							if ($email == $mail->toemail)
							{
								...
								$content .= "<p><iframe id='mp' style='width:100%;border:0;height:500px;' src='" . get_option('siteurl') . '/' . MP_PATH . "mp-includes/action.php?action=view&id=$mail_id&key=$view' onload='resizeIframe'></iframe></p>";
							}
							else
							{	// not a recipient
