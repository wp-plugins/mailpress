	...
	function mp_mail_links() {
		...
		switch (true)
		{
			case (isset($_GET['del'])) :
				...
				$del 		= $_GET['del'];
				$mp_user_id = MP_User::get_user_id($del);
				$email 	= MP_User::get_user_email($mp_user_id);

				if ('active' == MP_User::get_user_status($mp_user_id)) $active = true;
				...
				if ($mp_user_id)
				{
					...
					if ($ok)
					{
						$content .= "	<input type='hidden'                name='status' value='" . MP_User::get_user_status($mp_user_id) . "' />\n";
			...
			case (isset($_GET['delall'])) :
				$mp_user_id    = MP_User::get_user_id($_GET['delall']);
				$email = MP_User::get_user_email($mp_user_id);
				...
				if ($mp_user_id)
				{
					if (isset($_POST['delconf'])) 
					{
						if (MP_User::set_user_status($mp_user_id, 'delete'))
						{
			...
			break;
			case (isset($_GET['add'])) :
				$mp_user_id    = MP_User::get_user_id($_GET['add']);
				$email = MP_User::get_user_email($mp_user_id);
				...
				if ($mp_user_id)
				{
					if ('active' != MP_User::get_user_status($mp_user_id))
					{
						if (MP_User::set_user_status($mp_user_id, 'active'))
						{	
			...
			case (isset($_GET['view'])) :
				$view  = $_GET['view'];
				$mp_user_id    = MP_User::get_user_id($view);
				$email = MP_User::get_user_email($mp_user_id);	
