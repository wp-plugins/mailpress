...
//alert ('html(name : '+name+', id : '+id+', url : '+url+', ok : '+ok+')');
		var html = '';
...
//alert(xml);
	...
	add    : function() {
		...
		iframeurl = htmuploadL10n.iframeurl + '?action=iframe_upload&draft_id=' + draft_id + '&id=' + mp_fileupload.nbfiles;
...
//alert ('changed('+id+','+file+')');
		return true;
	},

	submitted : function(id, file) {
//alert ('submitted('+id+','+file+')');
...
//alert ('loaded('+id+','+filename+','+xml+','+oldid+')');
//alert(xml);
...
//alert('parsexml : '+id+' '+url+' '+file+' '+error);
